...
public class PooledFactory implements PooledObjectFactory {
	...
	public PooledFactory(String appId) {
		this.appId = appId;
	}
	...
	/**
	 * 判断资源对象是否有效 may be invoked on activated instances to make sure they can be borrowed from the
	 * pool. validateObject(org.apache.commons.pool2.PooledObject<T>) may also be used to test an
	 * instance being returned to the pool before it is passivated. It will only be invoked on an
	 * activated instance.
	 * @param p
	 * @return
	 */
	...
	/**
	 * activateObject is invoked on every instance that has been passivated before it is borrowed
	 * from the pool. 从资源池中取出已经创建过的池对象之前
	 * @param p
	 * @throws Exception
	 */
	@Override
