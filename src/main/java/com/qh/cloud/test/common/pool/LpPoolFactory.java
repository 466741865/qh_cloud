...
public class LpPoolFactory implements PooledObjectFactory<User> {
	...
	/**
	 * 功能描述：激活资源对象 什么时候会调用此方法
	 * @ 1：从资源池中获取资源的时候
	 * @ 2：资源回收线程，回收资源的时候，根据配置的 testWhileIdle 参数， 判断 是否执行 factory.activateObject()方法，true 执行，false
	 *   不执行
	 * @param arg0
	 */
	@Override
	...
	public PooledObject<User> makeObject() throws Exception {
		...
		User user = new User("li " + Thread.currentThread().getName(), 24, "178cm");
